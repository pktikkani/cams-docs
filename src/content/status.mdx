...
| Backend API | Go/Fiber, PostgreSQL, Redis | **Fully hardened** (43 bugs fixed, security audit complete) |
...
| Feature | Status |
|---------|--------|
| Production observability (zerolog + Prometheus + Grafana + Loki) | DONE |
| Docker Compose monitoring stack (6 services) | DONE |
| Grafana dashboards (API Overview + Security) | DONE |
| Grafana alert rules (6 automated alerts) | DONE |
...
| Backend security hardening (43 bugs fixed) | DONE |
| Per-device HMAC keys (256-bit, generated on login) | DONE |
| JWT claims hardening (iss/sub/aud, token type isolation) | DONE |
| Graceful shutdown (SIGTERM/SIGINT handler) | DONE |
| Health endpoint with DB/Redis connectivity checks | DONE |
| Metrics endpoint authentication (optional bearer token) | DONE |
| CORS origin restriction (configurable via env var) | DONE |
| Database performance indexes (4 missing indexes added) | DONE |
...
| Production observability stack | DONE | zerolog, Prometheus, Grafana, Loki |
| On-premise Oman deployment | IN PROGRESS | Docker Compose stack ready, blocked on DC access |

---

## Backend Security Audit (February 2026)

A comprehensive security audit of the Go backend identified and resolved **43 bugs** across all severity levels. All fixes are deployed to production.

| Severity | Found | Fixed | Examples |
|----------|-------|-------|----------|
| Critical | 4 | 4 | Hash chain race condition, HMAC secret exposure, unauthenticated QR endpoint |
| High | 10 | 10 | Unchecked type assertions, nonce fail-open, CORS wildcard, hardcoded DB creds |
| Medium | 14 | 14 | Scanner JWT 30-day expiry, unbounded exports, sequential dashboard queries |
| Low | 11 | 11 | Bcrypt cost mismatch, health endpoint no-op, Prometheus cardinality |
| DB Indexes | 4 | 4 | Missing indexes on scan_logs, access_schedules, devices |
| **Total** | **43** | **43** | |

### Key Hardening Changes

- **Per-device HMAC keys** — Each device gets a unique 256-bit key on login (no more shared secret)
- **JWT token type isolation** — Added `iss`, `sub`, `aud` claims; admin tokens can't be used on scanner endpoints
- **Nonce anti-replay fail-closed** — Redis errors now return 503 instead of allowing through
- **Hash chain serialization** — PostgreSQL advisory locks prevent race conditions on audit log appends
- **Offline bundle encryption** — TOTP seeds AES-256-GCM encrypted with scanner's device key before transmission
- **Graceful shutdown** — SIGTERM/SIGINT handler with 10s drain timeout
- **Input validation** — UUID validation on all path params, date/time format validation, device fingerprint SHA-256 format check, batch size limits
- **Health checks** — `/health` now pings PostgreSQL and Redis, returns 503 with degraded status
- **Metrics auth** — `/metrics` protected by optional `METRICS_TOKEN` bearer auth

See the full [BACKEND-BUGS.md](https://github.com/pktikkani/contractor-qr-api/blob/main/BACKEND-BUGS.md) for detailed descriptions of all 43 fixes.

---

## Known Bugs

No known bugs. All previously reported issues have been resolved.

| Bug | Severity | Component | Status |
|-----|----------|-----------|--------|
| ~~QR code distortion on site switch~~ | Low | iOS Contractor | **FIXED** |
| ~~43 backend security bugs~~ | Critical-Low | Backend API | **ALL FIXED** |
...
### Phase 3 — Hardening
- ~~Backend security audit (43 bugs fixed)~~ **DONE**
- ~~Per-device HMAC keys~~ **DONE**
- ~~JWT token type isolation~~ **DONE**
- ~~Graceful shutdown + health checks~~ **DONE**
- Rate limiting (Go Fiber middleware)
- Android biometric auth
- Arabic/RTL for remaining apps
- Dependency vulnerability remediation (automated via Dependabot)

### Phase 4 — Integration & Deployment
- Oracle HRMS integration (blocked on credentials)
- On-premise Oman data center migration (Docker Compose stack ready)
- Load testing and penetration testing
- Production Grafana alert tuning and notification channels (Slack/email)
